...
if command -v nvidia-smi &> /dev/null && nvidia-smi >/dev/null 2>&1; then
  ...
  if [ -n "${NVRTC_ARCHIVE}" ]; then
    if cd /tmp && curl -fsSL "${NVRTC_URL}${NVRTC_ARCHIVE}" | tar -xJf - -C /tmp; then
      mv -f cuda_nvrtc* cuda_nvrtc && cd cuda_nvrtc/lib && chmod -f 755 libnvrtc* && rm -f "/opt/gstreamer/lib/${GST_ARCH}/"libnvrtc* && mv -f libnvrtc* "/opt/gstreamer/lib/${GST_ARCH}/" && cd /tmp && rm -rf /tmp/cuda_nvrtc && cd "${HOME}"
    else
      echo "WARN: NVRTC download/extract failed, continuing without NVRTC."
    fi
  fi
fi
...
}" | tee /etc/nginx/sites-available/default > /dev/null || true
